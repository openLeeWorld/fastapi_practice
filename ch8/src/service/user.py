...
MAYBE_KEY = "ignore this from security, github"
# 실제로는 하드코딩이 아니라 환경 변수 파일에서 설정해야 한다.
ALGORITHM = "HS256"
...
def get_jwt_username(token: str) -> str | None:
    ...
    try:
        payload = jwt.decode(token, MAYBE_KEY, algorithms=[ALGORITHM])
...
def create_access_token(data: dict, expires: timedelta | None = None):
    ...
    encoded_jwt = jwt.encode(src, MAYBE_KEY, algorithm=ALGORITHM)
    return encoded_jwt
